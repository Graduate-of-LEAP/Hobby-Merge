...
export default function Profile() {
  ...
  return (
    <div className="flex flex-col items-center w-full h-full justify-center gap-5 bg-blue-100">
      <img alt="Profile" width={200} height={200} className="rounded-full">
        {user?.profilePicture}
      </img>
      <div>
        <DropdownMenu>
          ...
          <DropdownMenuContent className="w-56 mb-4 mr-48 bg-slate-600 text-teal-50  flex flex-col gap-5 p-4 rounded ">
            <input
              type="file"
              accept="image/*"
            ...
            <DropdownMenuGroup className="flex items-center gap-5 ">
              <Button
                ...
                className="font-large bg-transparent"
              >
                Remove Picture
              </Button>
            </DropdownMenuGroup>
      ...
      <p>
        ...
        {user?.hobby.map((item, i) => (
          <span
            className="w-fit h-fit bg-blue-300 border rounded-full p-2"
            key={i}
          >
            {item.name}
          </span>
