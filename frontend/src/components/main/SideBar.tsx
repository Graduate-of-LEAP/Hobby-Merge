        ...
        <div className='w-[260px] flex flex-col justify-between'>
            <div className='w-[260px] text-xl flex flex-col items-center py-4 bg-[#d8ebf5] rounded-xl'>
                ...
                <div className='flex flex-col justify-center'>
                    ...
                    <p className='text- text-[#6c767b]'>@Dulguun...</p>
                </div>
            </div>
            <div className='w-full flex flex-col gap-3 p-5   '>
                {paths.map((item, index) => {
                    return (
                        <button
                            key={index}
                            className={`w-[204px] ${pathname === item.path ? "bg-white" : "bg-[#d8ebf5]"} h-[38px] border rounded-2xl hover:bg-[#d8ebf5] hover: flex items-center justify-center gap-3 font-bold`}>
                            {item.icon}   {item.name}
                        </button>
                    )
                })}
            </div>
            <div className='w-full h-[320px] border  bg-[#d8ecf7] flex flex-col gap-3 p-4 rounded-xl'>
                Active status
                        ...
                        return (
                            <div key={index} className='w-[162px] h-[68px] rounded-xl flex gap-3 relative'>
                                ...
                                <div className='flex justify-center items-center '>
                                    <h1 className='text-l font-bold'>{item.name}</h1>
                                </div>
                            </div>
                        )
                    })}
                ...
                <div className='text-sm flex items-center'>
                    <input
                        className='w-[204px] h-[40px] border bg-white rounded-xl'
                        placeholder='search'
                        value={searchValue}
